...
from src.stage_3_1_structured_mlp_cql.networks import Actor # We only need the Actor for evaluation
...
def main(args):
    ...
    model = Actor(config)
    checkpoint_path = 'runs/CQL_MLP_2025-08-07_07-50-00_lr3e-05_gamma0.95_cqlalpha10.0/models/cql_mlp_policy_epoch_47.pth'
...
# 
        ...
        with torch.no_grad():
            ...
            # --- FIX 1: Unpack the tuple from the forward pass ---
            mean, _ = model(state_dict_tensor)
            
            # --- FIX 2: Use the Actor's `sample` method for rescaling ---
            # A cleaner way is to just take the mean and then rescale it.
            # The Actor's `sample` method does tanh and rescaling.
            # To get the deterministic action, we can pass the mean through this process.
            y_t = torch.tanh(mean)
            action_rescaled = y_t * model.action_scale + model.action_bias
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate a trained Structured MLP CQL policy (Stage 3.1).")

    parser.add_argument("-s", "--scenario_id", type=str,
                        help="The scenario ID to evaluate. If not provided, one will be chosen randomly.")
    parser.add_argument(
        "--output_dir", type=str,
        default=os.path.expanduser('~/WaymoOfflineAgent/outputs/evaluations/stage_3_1_structured_mlp_cql'),
        help="Directory to save the output video."
    )
    args = parser.parse_args()

    # Instantiate config to access val_data_dir
    config = TrainingConfig()

    # If scenario_id is not provided, pick one randomly
    if args.scenario_id is None:
        all_npz_files = [f for f in os.listdir(config.val_data_dir) if f.endswith('.npz')]
        if not all_npz_files:
            raise RuntimeError("No .npz files found in validation data directory.")
        args.scenario_id = random.choice(all_npz_files).replace('.npz', '')
        print(f"[INFO] No scenario_id provided. Randomly selected: {args.scenario_id}")
