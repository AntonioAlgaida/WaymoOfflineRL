# ======================================================================================
# train.py (Stage 2.7 - Transformer BC)
#
# Description:
#   Trains the Structured Behavioral Cloning (BC-S) agent using a transfomer architecture. It uses a dataset
...
from src.stage_2_7_transformer_bc.networks import TransformerBCModel # Import our new transformer-structured model
...
def train(config: TrainingConfig):
    ...
    os.makedirs(os.path.dirname(config.bcs_t_model_path), exist_ok=True)
    ...
    model = TransformerBCModel(config).to(config.device)
    
    # Load previous checkpoint if it exists
    if os.path.exists(config.bcs_t_model_path):
        print(f"Loading existing model from {config.bcs_t_model_path}")
    ...
    for epoch in range(config.num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), config.bcs_t_model_path)
            print(f"  -> New best model saved to {config.bcs_t_model_path} (Val Loss: {best_val_loss:.6f})")
