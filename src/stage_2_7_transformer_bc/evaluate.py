# ======================================================================================
# evaluate.py (Stage 2.7 - Transformer BC)
#
# Description:
#   Loads the trained Transformer-based Behavioral Cloning (BC-T) policy and
...
# --- KEY CHANGE: Import the Transformer model ---
from src.stage_2_7_transformer_bc.networks import TransformerBCModel
...
# --- Live Feature Engineering (This function can be shared across evaluations) ---
# NOTE: This function is identical to the one in Stage 2.6's evaluate.py
...
def main(args):
    print("--- Starting Stage 2.7: Transformer BC Policy Evaluation ---")
    config = TrainingConfig()

    # --- 1. Load Scenario and Environment (Identical to Stage 2.6) ---
    ...
    env_config = _config.EnvironmentConfig(
        max_num_objects=initial_state.num_objects, controlled_object=_config.ObjectType.SDC,
        metrics=_config.MetricsConfig(), rewards=_config.LinearCombinationRewardConfig(rewards={})
    )
    dynamics_model = dynamics.InvertibleBicycleModel()
    env = _env.MultiAgentEnvironment(dynamics_model=dynamics_model, config=env_config)

    # --- 2. Load Trained Transformer Model ---
    # --- KEY CHANGE: Use the correct model path from config and the TransformerBCModel class ---
    print(f"Loading trained model from: {config.bcs_t_model_path}")
    model = TransformerBCModel(config) # Instantiate the Transformer model
    model.load_state_dict(torch.load(config.bcs_t_model_path, map_location=config.device))
    model.to(config.device)
    model.eval()

    # --- 3. The Simulation Loop (Identical to Stage 2.6) ---
    ...
    # --- 4. Visualization (Updated Filename) ---
    print("Generating rollout video...")
    imgs = []
    for state in tqdm(rollout_states, desc="Rendering frames"):
        imgs.append(visualization.plot_simulator_state(state, use_log_traj=False))

    # --- KEY CHANGE: Use a new filename for the output video ---
    output_path = os.path.join(args.output_dir, f"{args.scenario_id}_bct_rollout.mp4")
    mediapy.write_video(output_path, imgs, fps=10)
    print(f"--- ✅ Transformer BC Evaluation Complete. Video saved to: {output_path} ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate a trained Transformer BC policy (Stage 2.7).")
    parser.add_argument("scenario_id", type=str, help="The scenario ID to evaluate.")
    parser.add_argument(
        "--output_dir", type=str,
        default=os.path.expanduser('~/WaymoOfflineAgent/outputs/evaluations/stage_2_7_transformer_bc'),
        help="Directory to save the output video."
    )
    args = parser.parse_args()
    os.makedirs(args.output_dir, exist_ok=True)
    
    # NOTE: Before running, ensure `utils.py` contains the `state_to_feature_dict` function
    # and a new path `bct_model_path` in the `TrainingConfig`.
    main(args)