...
# Import our new Transformer-based networks
from src.stage_3_offline_rl.networks import TransformerActor, TransformerCritic
...
class CQLTrainer:
    ...
    def __init__(
        ...
        # --- Networks (Now using Transformer models) ---
        self.actor = TransformerActor(config, action_dim=config.action_dim).to(device)
        self.critic = TransformerCritic(config, action_dim=config.action_dim).to(device)
        self.critic_target = deepcopy(self.critic)

        # --- Optimizers ---
        self.actor_optimizer = torch.optim.AdamW(self.actor.parameters(), lr=self.lr, weight_decay=1e-5)
        self.critic_optimizer = torch.optim.AdamW(self.critic.parameters(), lr=self.lr, weight_decay=1e-5)
